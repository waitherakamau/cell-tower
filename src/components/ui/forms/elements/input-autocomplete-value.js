...
export default class Autocomplete extends React.Component {
	...
	componentDidMount () {
		...
		if(value){
			let dataExists     = options.find ( e => e.value === value )
			this.setState({ userValue: dataExists?.name || "" })
		}
	}
	onBlur (e, inputV){
		const {
		...
		console.log('obLur', e, inputV)

		if ( !isValid ) {
			updateState ( inputId, formId, { hasError: true, input: value, message: `Invalid ${inputId}` })
		}
		else {
			updateState ( inputId, formId, { hasError: false, input: value, message: '' })	
	...
	onChange (event, { newValue }) {
		...
		let iputVal = newValue
		if(typeof newValue !== 'string'){
			iputVal = newValue.name
		}
		console.log(newValue)
		updateText ( inputId, formId, { textInput: newValue.value })

		this.setState({
			userValue: iputVal
		})
	}	
	onSuggestionsFetchRequested (itemValue) {
		...
		console.log('onSuggestionsFetchRequested', itemValue)
		let suggestions = getSuggestions ( itemValue.value, this.props.options )
		let hasError = suggestions.length === 0
		

		//updateState ( inputId, formId, { hasError, input: itemValue.value, message: '' })

		this.setState({
			suggestions: getSuggestions ( itemValue.value, this.props.options )
		})
	}
    ...
    render() {
        const { suggestions, userValue } = this.state
